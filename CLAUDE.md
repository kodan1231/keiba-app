...
- **テーブル全件を集計・グルーピングしたJSONを事前計算キャッシュとして持つ場合、
  「1行固定」で持ってはいけない。必ず最初からチャンク分割(複数行)で設計する**。
  D1には「1行(1カラム値)あたり2,000,000バイト」の上限があり、元テーブルが育つ限り
  1行固定のJSONキャッシュはいずれ必ずこの上限に抵触する。`races_cache`(2026-09-12。
