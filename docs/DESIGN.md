...
- 降着・失格など、取消・除外・中止以外の特殊な着順未確定ケースは今回対象外
  (`docs/BACKLOG.md`参照)
- **返還(refund)処理は今回対象外**: 「返還馬番」「返還同枠」の情報自体は
  `jraResultParseRefund()`で抽出され`races.payouts.refunds`に保存されるが、この情報を
  使って該当する組み合わせ(取消・除外・中止馬が絡む枠連・馬連等)の払戻を「不的中(0円)」
  ではなく「返還(全額払戻)」として計算する仕組みは無い(`recomputeTicketPayoutsForRace`は
  `refunds`を一切参照していない)。取消・除外・中止に該当する馬の枠番が未確定(null)の
  場合は枠連が「判定不能(未確定のまま)」になる既存ロジックで一部緩和されるが、馬番が
  確定している式別(馬連・ワイド・馬単等)ではこの問題を回避できない。設計・実装は
  別タスクとして`docs/BACKLOG.md`に記録する
