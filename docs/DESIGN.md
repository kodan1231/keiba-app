...
2026-08-16(2): トップページ(`/`)を「馬券購入」画面にする対応方式を、middlewareでのHTTP
リダイレクトから、ファイル名リネーム方式に変更(詳細は下記「トップページ(`/`)の表示について」参照)。
2026-08-16(3): 確定済みレースへの購入時に払戻を即時反映するよう変更。
2026-08-16(4): 騎手名エイリアス管理(jockey_aliases)を新規実装
2026-08-20: 予想登録画面の馬メモ有無の表示方法・一覧の行の高さ・ヘッダー余白を見直し。
詳細は下記「予想登録画面(`prediction.html`)」参照)
2026-08-20(2): 馬券履歴画面(`history.html`)の「選択解除」ボタンを廃止(同じ日付の
再クリックで解除できる機能と重複していたため)
2026-08-21: 馬券履歴画面(`history.html`)を、初期表示から当日を選択済みにし、必ず
どこかの日付が選択された状態を保つ(再クリックでの解除も廃止)よう変更。あわせて
サマリー表示を全期間集計から選択中の日付の合計へ変更。詳細は下記
「購入履歴画面(`history.html`)」参照)
2026-08-22: 馬券購入画面の「開催日・レースを選ぶ」見出しを削除。馬券履歴画面の
...
- **表示時の組み立て**: `public/utils.js`の`formatCourseText(courseType, distance)`が、
  ...
  いた関数を`utils.js`へ移動し、`races.html`・`index.html`双方から共有する形に統一した
...
- `public/jra-entries-pdf.js`: クライアント側のPDF.js解析(全角英数記号・康熙部首/CJK部首補助の
  ...
  **2026-08-14修正**: 以前は`.normalize("NFKC")`を使っていたが、CJK互換漢字(人名用の
  ...
  置き換えた。**2026-08-16修正(実装待ち・詳細は下記「全角文字正規化の方針」参照)**:
  上記2026-08-14修正の副作用として、「日」「月」「発」「走」「馬」等が康熙部首・CJK部首
  補助ブロックの文字としてPDFから抽出されるケースで日付・発走時刻等の検出が0件になる
  回帰が発生したため、当該ブロックのみを対象にした正規化を追加する。パーサーバージョンは
  `1.4.0-radical-fix`に更新予定)。**性齢・負担重量の抽出は
...
- **康熙部首・CJK部首補助の正規化を追加(2026-08-16修正・実装待ち)**: 上記2026-08-14修正の
  ...
  (解析結果0レース)になる回帰不具合**が発生していた(JRAレース結果PDFインポート・
  出走馬一覧PDFインポートの両方で確認)。
  詳細は下記「全角文字正規化の方針(2026-08-16修正)」参照。パーサーバージョンは
  `9.1.0-preserve-name-glyphs` → `9.2.0-radical-fix`に更新予定
...
- **全着順・タイム等の抽出(2026-08-11追加・実装済み)**: 下記「レース結果の詳細記録
  (race_results)」参照
...
標準の漢字へ変換する処理を追加する(`jraResultNormalizeUnit()`/`jraEntriesNormalizeUnit()`
...
更新する(`jra-result-pdf.js`: `9.1.0-preserve-name-glyphs` → `9.2.0-radical-fix`、
`jra-entries-pdf.js`: `1.3.0-preserve-name-glyphs` → `1.4.0-radical-fix`)。
...
### トップページ(`/`)の表示について(2026-08-16(2)改定)

Cloudflare Pages(および一般的な静的サイトホスティング)の標準仕様として、サイトのルートURL
(`/`)へアクセスすると`index.html`が返される。これはアプリケーション側で制御できない挙動である。

以前は`index.html`が「購入履歴」画面であり、ルートURLを開くと購入履歴が表示されてしまって
いた。これを「馬券購入」画面にするため、2026-08-16に一度`functions/_middleware.js`で`/`への
アクセスを`/buy.html`へHTTPリダイレクトする対応を行ったが、Cloudflare Pagesには
「`*.html`付きURLへのアクセスを拡張子なしURLへ自動的に308リダイレクトする」という別の標準
挙動があり、この2つの挙動が衝突して意図しないリダイレクトの連鎖(`/index.html`→`/`→
`/buy.html`→`/buy`)が発生し、「馬券履歴」ナビリンクをクリックしても馬券購入画面に
遷移してしまう不具合が生じていた(2026-08-19発覚)。

**この問題を回避するため、middlewareによるHTTPリダイレクトは廃止し、代わりにファイル名を
実際の画面の役割に合わせてリネームする方式に変更した(2026-08-16(2)再修正)。**

| 変更前 | 変更後 | 画面 |
|---|---|---|
| `public/buy.html` | `public/index.html` | 馬券購入(レース選択・本日の日付が初期選択された状態) |
| `public/index.html`(旧・購入履歴) | `public/history.html` | 馬券履歴(購入履歴) |

これにより、サイトのルートURL(`/`)へアクセスすると、Cloudflare Pagesの標準挙動により
そのまま`index.html`(＝馬券購入画面)が返される。`functions/_middleware.js`によるルートURLの
特別なリダイレクト処理は不要になったため削除した(`/api/*`の既存の認証チェック処理には
影響しない)。

- ヘッダーの「馬券履歴」ナビリンクは`history.html`を指す(旧`index.html`の内容そのまま)
- 各画面からの「馬券購入」ボタン・リンク(`prediction.html`の「このレースの馬券を購入」等)は
  `index.html`(旧`buy.html`)を指すよう更新した(例: `index.html?race=ID`でレースIDを指定して
  直接購入モーダルを開く動作は従来通り)
- スクリプトファイル名(`buy.js`・`app.js`)は変更していない。`index.html`が`buy.js`を、
  `history.html`が`app.js`を読み込む対応関係になる(README「構成」のファイル一覧に明記)
- **注意**: `index.html`というファイル名と「馬券購入」という画面、`history.html`という
  ファイル名と「馬券履歴」という画面は、今回一致させたため分かりやすくなったが、
  `buy.js`(index.htmlが読み込む)・`app.js`(history.htmlが読み込む)のようにJSファイル名
  とHTMLファイル名は一致していない箇所が残っている。変更が必要な場合は、この対応関係を
  README「構成」・本節に必ず追記すること
...
常にカレンダーを表示する。**2026-08-22改定**: 画面上の表示順を「カレンダー→サマリー」に
変更した(以前は「サマリー→カレンダー」の順だった)。まずカレンダーで日付を選び、その結果
としてのサマリーを下に確認する、という操作の流れに沿うように並びを変更した。HTML構造の
入れ替えのみで、JS側(`public/app.js`)は要素IDで参照しているため挙動に変更は無い。

**2026-08-21改定**: 画面を開いた直後から当日が選択済みの状態になっており、その日の購入履歴
(詳細)がすぐに表示される(以前は日付未選択が初期状態で、「カレンダーから日付を選ぶと〜」
という案内文のみが表示されていたが、この初期状態自体を廃止した)。**必ずどこかの日付が
選択されている状態を保つ**方針とし、選択中の日付を再クリックしても解除されない(以前あった
「同じ日付の再クリックで選択解除」機能も2026-08-21に廃止した。専用の「選択解除」ボタンは
2026-08-20に廃止済み)。**この結果、「カレンダーから日付を選ぶと、その日の購入履歴が
表示されます」という案内文(初期状態専用のメッセージ)が表示される場面が無くなったため、
2026-08-22に該当要素ごと削除した。**
サマリー(総購入・総払戻・収支・回収率)は、**選択中の日付の合計のみ**を表示する
(以前は選択日にかかわらず常に全期間を集計していたが、`stats.html`の「総合成績」
(全期間の同じ4項目)と表示内容が重複していたため、この画面では「選択した日の収支」を
確認する用途に絞った)。カレンダー日別マス目の収支表示は実数(`toLocaleString()`、
例: `+12,345`)で表示する。選択日の履歴はレース単位(`groupByRace`)にまとめて表示し、
日付降順・同日内は競馬場名(ロケール比較)・レース番号の昇順でソートする。購入履歴の
削除成功時は`alert()`を出さず、削除直後に`loadTickets()`で即座に一覧へ反映する
(失敗時のエラーメッセージは`alert()`で表示する)。「今日」ボタンは表示月を当月に
切り替え、今日の日付を選択状態にして履歴も絞り込む。フィルタの解除は、選択中の日付をもう一度クリックする操作のみで行う
(**2026-08-20: 専用の「選択解除」ボタンは廃止した**。同じ日付の再クリックで解除できる
機能と重複していたため。レース管理画面(`races.html`)のカレンダーも同様の考え方で、
専用の解除ボタンは元々設けていない)。PC(幅`700px`超)ではCSVインポートボタンを表示し、
それ以下では非表示にする(機能・APIは維持したまま表示のみ制御)。
...
- 競馬場別: 購入金額の多い順
- 騎手別: **回収率(rate)の高い順**(2026-08-22変更。以前は購入金額の多い順だった。
  回収率が未確定(null)の行は、ソート方向によらず常に末尾に表示される既存仕様のまま)

数値列(購入/払戻/収支/回収率/的中率)は初回クリックで降順、文字列・日付列(先頭列)は
初回クリックで昇順から始まる。値がnull(未確定で回収率・的中率が算出できない行)は
ソート方向によらず常に末尾に表示する。

騎手別収支には、以前「1枚の馬券に複数の騎手が関わる場合(馬連・三連単など)、関係する騎手
それぞれに購入金額・払戻金の全額を計上しています。そのため合計は購入履歴全体の総額と
一致しません。」という説明文を表示していたが、**2026-08-22に削除した**(集計ロジック自体
(按分せず全額計上する仕様)に変更はない。`docs/DESIGN.md`「データの考え方」に記載の
「騎手別の収支は、1枚の馬券に複数騎手が関わる場合、関係する騎手それぞれに購入金額・払戻金の
全額を計上する(按分しない)」という仕様は維持しており、画面上の補足説明のみを省略した)。
...
- サイトのルートURL(`/`)へアクセスすると、この画面が表示される(未ログイン時はログイン
  画面が先に表示される)。上記「トップページ(`/`)の表示について」参照
- **2026-08-20〜**: 「開催日・レースを選ぶ」見出しの下にあった説明文(「レースを選択すると
  予想ページを開きます。予想を固めた後、「このレースの馬券を購入」から購入内容を作成します。」)
  は冗長と判断し削除した(操作フロー自体に変更はない)
- **2026-08-22〜**: 「開催日・レースを選ぶ」という見出し自体(`<h2>`)も、上記の説明文削除に
  続いて冗長と判断し削除した。見出しが無くても、画面を開けばすぐにカレンダーとレース一覧が
  表示されるため、操作フローへの影響は無い
...
- 「このレースの馬券を購入」ボタンはヘッダー内(レース名の近く)に配置し、出走頭数が多い
  レースでも下までスクロールせずに購入画面へ進める(リンク先は`index.html?race=ID`)
...
- 馬メモ入力欄には「次回以降も参照したい、この馬固有のメモを入力」というプレースホルダー説明文を表示しない。
- **馬メモの有無の示し方(2026-08-20改定)**: 各馬の行を開かなくても、メモの有無が
  ひと目で分かるようにする。**開閉トグル(「＋」/「－」)の隣に「▼」マーク**
  (`.note-toggle-tail`内。購入画面のレース選択グリッドで使っている`.memo-mark`と
  同じスタイルを流用)を表示し、メモが登録されている馬だけに付く。以前は行全体の背景色
  強調(`.horse-note-card.has-horse-memo`)・馬名横の赤枠「メモ」バッジ・馬名下のメモ抜粋
  (プレビュー文字)の3つを併用していたが、いずれも廃止した。抜粋表示は行の高さが
  メモの有無で変わってしまう原因になっており、行の高さを詰めたい・一覧性を高めたいという
  要望と相性が悪かったため、`▼`マークのみに一本化した。
  - **不具合修正(2026-08-20)**: `public/prediction.js`の`renderHorses()`が、メモの有無を
    `horseNotes[e.horse_name]`(出走馬表側の生の馬名)で判定していたが、`horseNotes`
    (`GET /api/horse-notes?race_id=...`のレスポンス)のキーは登録時に空白正規化された
    馬名になっているため、空白の入り方の違いで一致せず「メモは登録されているのに
    何も表示されない」不具合があった。`normalizeHorseName()`で正規化してから参照する
    よう修正した。
...
- **操作説明文の廃止(2026-08-20)**: 「予想印は馬名の横で変更できます。馬をクリックすると、
  この馬についてのメモだけを入力できます。印とメモは変更時に自動保存されます。」という
  操作説明文(`.prediction-help`)は、画面幅を問わず表示しないこととした(以前はモバイル
  (600px以下)のみ非表示・PCでは常時表示だった)。要素自体をHTMLから削除している
- **一覧の行の高さ・ヘッダー余白の削減(2026-08-20)**: フルゲート(18頭)のレースで、
  スクロールなしでは一覧の一部(例: 15番以降)が画面外に隠れてしまうという指摘を受け、
  一覧性を高めるため以下を詰めた。メモの有無に関わらず全馬の行に適用する。
  - 馬の行(`.horse-note-toggle`)のpaddingを12px→9px(430px以下は10px 8px→7px 8px)に削減
  - ヘッダー(`.prediction-race-header`)のpaddingを18px→14px(横方向は18pxのまま)に削減
...
- レースの着順・払戻が確定した後の**新規購入**も引き続き可能。予想画面の「このレースの
  ...
  レースを開くとレース名見出しの横に「確定済」バッジを表示するが、購入UI自体はブロック
  しない(**2026-08-20〜**: 以前は案内バナー(`#race-settled-block`)を表示していたが、
  レース名見出し横のバッジ(`#race-settled-badge`)表示へ変更した。バナー文言は廃止した)
...
`YYYYMMDDHHmm`形式へ揃えることが望ましい。**なお2026-08-16(2)のファイルリネーム
(`buy.html`→`index.html`等)では、CSS自体(`style.css`)は変更していないため`v=`の値は
更新していない(誤って`v=3`にした後、`v=2`に戻した)。**2026-08-20: カレンダーツールバー
(前月・次月・今日ボタン)のモバイル縮小対応で`style.css`を初めて実変更したため、
全HTMLファイルの`v=`を`2`→`3`へ更新した。**
**2026-08-20の予想登録画面改修
(馬メモ表示・行高さ・ヘッダー余白)でも`style.css`を変更しているが、`v=`の値は
引き続き更新していない(次回のCSS変更時にまとめて`YYYYMMDDHHmm`形式へ揃えることを推奨)。**
**2026-08-22の見出し削除・カレンダー/サマリー入れ替え・案内文削除・騎手別収支の初期ソート
変更・説明文削除はいずれもCSS(`style.css`)を変更していないため、`v=`の値は更新していない。**
...
**2026-08-16(2)のファイルリネーム(`buy.html`→`index.html`、旧`index.html`→`history.html`)
はいずれも`public/`配下の静的ファイルの変更のみで、DBスキーマの変更は伴わない。
`migration.sql`・`schema.sql`とも今回の変更対象外。**

**2026-08-20の予想登録画面改修(馬メモ有無の表示方法変更・不具合修正・行高さ/ヘッダー余白の
削減)も`public/prediction.js`・`public/prediction.html`・`public/style.css`のみの変更で、
DBスキーマの変更は伴わない。`migration.sql`・`schema.sql`とも今回の変更対象外。**

**2026-08-22の見出し削除・カレンダー/サマリー入れ替え・案内文削除・騎手別収支の初期ソート
変更・説明文削除も`public/index.html`・`public/history.html`・`public/stats.html`・
`public/stats.js`のみの変更で、DBスキーマの変更は伴わない。`migration.sql`・`schema.sql`
とも今回の変更対象外。**
...
- **PC(`700px`超)**: レースカードのヘッダーは、左側に「日付・競馬場・レース番号」を配置し、
  右側に「購入額・払戻額・収支」を配置する(1行)。レース名は既存表示を維持し、左側のレース
  情報に続けて表示する。
- **モバイル(`700px`以下・2026-08-16〜)**: 縦長になりすぎるのを防ぐため、ヘッダーを2行に
  再構成する。詳細は上記「購入履歴画面(`history.html`)」の「レースカードの表示」節を参照。
  要点のみ再掲する:
  - 1行目: 競馬場・レース番号・レース名(日付は非表示。レース名は画面幅超過時のみ
    `ellipsis`省略)
  - 2行目: 購入額・払戻額・収支を左寄せで表示し、行末に開閉矢印(▸/▾)を表示する
  - HTML構造はPC・モバイル共通で、CSSの`order`・`flex-basis`・`margin-right:auto`のみで
    見た目を切り替える(`public/app.js`の`renderRaceCard()`・`public/style.css`の
    `@media (max-width: 700px)`ブロック)
