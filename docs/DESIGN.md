...
常にカレンダーを表示する。**2026-08-21改定**: 画面を開いた直後から当日が選択済みの状態に
なっており、その日の購入履歴(詳細)がすぐに表示される(以前は日付未選択が初期状態で、
「カレンダーから日付を選ぶと〜」という案内文のみが表示されていたが、この初期状態自体を
廃止した)。**必ずどこかの日付が選択されている状態を保つ**方針とし、選択中の日付を
再クリックしても解除されない(以前あった「同じ日付の再クリックで選択解除」機能も
2026-08-21に廃止した。専用の「選択解除」ボタンは2026-08-20に廃止済み)。
