...
2026-08-17: 購入画面のレース選択グリッド行(`.race-column-row`)のPC表示が、コース情報等の
幅に押されてレース名がほぼ潰れる不具合を修正(モバイルと同じ2行構成へ統一。実装完了)。
騎手名エイリアス一覧の並び順を登録日時順から表記ゆれ側(alias_display)の文字列昇順へ変更
(実装完了))
...
- 降着・失格など、着順が確定しない特殊ケースのうち**取消・除外・中止以外**
  (`race_results.status`の`demoted`/`disqualified`等)は今回対象外。2026-08-19時点で
  `finished`/`scratched`(取消)/`excluded`(除外)/`stopped`(中止)の4種類に対応する
  (中止対応は2026-08-19追加)
...
- `public/jra-entries-pdf.js`: クライアント側のPDF.js解析(全角英数記号・康熙部首/CJK部首補助の
  ...
  **2026-08-14修正**: 以前は`.normalize("NFKC")`を使っていたが、CJK互換漢字(人名用の
  ...
  置き換えた。**2026-08-16修正**: 上記2026-08-14修正の副作用として、「日」「月」「発」
  「走」「馬」等が康熙部首・CJK部首補助ブロックの文字としてPDFから抽出されるケースで
  日付・発走時刻等の検出が0件になる回帰が発生したため、当該ブロックのみを対象にした
  正規化を追加した(実装済み。パーサーバージョン`1.4.0-radical-fix`)。**性齢・負担重量の
  抽出は実装済み(騎手名/調教師名の境界判定で使っている
  `(牡|牝|せん|セ|騸)\s*(\d{1,2})\s+[\d.]+kg`のマッチ結果を利用できる。抽出結果を
  `entries`へ含める処理まで実装済み)**
...
- **康熙部首・CJK部首補助の正規化を追加(2026-08-16修正・実装済み)**: 上記2026-08-14修正の
  ...
  (解析結果0レース)になる回帰不具合**が発生していた。実装は完了済み(該当2ブロック全体を
  対象に1文字ずつNFKC正規化)。詳細は下記「全角文字正規化の方針」参照。パーサーバージョンは
  `9.2.0-radical-fix`
...
- **全着順・タイム・着差等の抽出(2026-08-11追加・実装済み)**: 下記「レース結果の詳細記録
  (race_results)」参照
- **取消・除外・中止馬の抽出(2026-08-11追加、中止対応は2026-08-19追加・実装済み)**:
  `取消 17 ラルス 牡3 55.0 △柴田 裕一郎 吉田 直弘`(馬体重なし)・`除外 13 ピアス 牝3 55.0
  黛 弘人 458 (-4) 森 一誠`(馬体重あり)のように、行頭が「取消」「除外」で始まる行は、
  着順・タイム等を持たない特殊行として`race_results`へ`status='scratched'`(取消)/
  `status='excluded'`(除外)で登録する。行頭が「中止」で始まる行(例: `中止 7
  ヒロイックヴァース 牡3 55.0 横山 武史 2 2 7 480 (+4) 浅利 英明 3`)は競走中止馬として
  `status='stopped'`で登録する(詳細は下記「レース結果の詳細記録」の「取消・除外・中止の
  扱い」参照)。**これら取消・除外・中止の馬も、完走馬と同様に`races.entries`(出走馬
  リスト)へ追加するようになった(2026-08-19変更)**。以前は完走馬のみを`entries`に
  追加していたため、出走馬情報をインポートせず結果PDFのみをインポートした場合、
  取消・除外・中止馬が`entries`から漏れて出走頭数が実際より少なく計算され、
  枠番の目安計算(`defaultWakuNumber()`)や予想登録・購入画面の頭数・馬番一覧に
  ズレが生じる不具合があった。この修正により`entries`の頭数は常に実際の出走頭数
  (取消・除外・中止を含む)と一致するようになる。取消・除外・中止馬も`horse_number`が
  確定していれば通常の馬と同様に予想登録・購入画面に表示され、馬券を購入できる
  (レースの結果が確定する前に馬券を購入する、という現実の購入行動と整合させるための
  意図的な仕様。的中し得ない馬券を買えてしまうこと自体は制限しない)
...
**修正内容(実装済み)**: 全角英数記号の半角化に加えて、**康熙部首(U+2F00–2FD5)・CJK部首補助
...
再発させることなく、日付・発走時刻等の検出不具合を解消できている。
...
テキスト抽出結果(診断パネルの「PDF全文抽出テキスト」)を確認した上での対応。実ブラウザでの
再検証結果はまだこのドキュメントに追記されていないため、確認が取れ次第反映すること。
...
- パーサー(`jra-result-pdf.js`)の行内テキスト連結が、出走馬一覧PDFインポート側で
  ...
  `docs/BACKLOG.md`参照)
- 降着・失格など、取消・除外・中止以外の特殊な着順未確定ケースは今回対象外
  (`docs/BACKLOG.md`参照)
...
- `public/jra-result-pdf.js`: クライアント側の解析ロジック本体(パーサーバージョン
  `9.3.0-stopped-status`。2026-08-19: 「中止」馬の解析対応、および取消・除外・中止馬を
  `races.entries`へ追加する変更を含む)
...
### 取消・除外・中止の扱い(2026-08-19: 中止への対応を追加)
...
- 一覧表示(表記ゆれ側・正しい表記・登録日時・削除ボタン)。**一覧の並び順は
  「表記ゆれ側(`alias_display`)の文字列昇順」で表示する(2026-08-17変更・実装完了。
  以前は登録日時の新しい順だった)。表記ゆれ側の文字列比較は、SQLiteの標準的な
  バイト順比較で行う(このアプリの他画面の名前列ソート(`stats.js`)と同じ方式に揃える。
  厳密な五十音順(濁点・拗音等の正規化)までは行わない)。同じ表記ゆれ側の値が複数ある
  場合は、登録日時の新しい順を副次キーとして使う**
...
- API: `GET/POST /api/admin/jockey-aliases`・`DELETE /api/admin/jockey-aliases/:id`・
  ...
  (`requireAdmin()`)。**`GET`のソート順は上記の通り`alias_display`昇順(実装完了)**
...
- `functions/_middleware.js`(サイト全体の全リクエストに対して呼び出される
  ...
  (`/api/*`の既存の認証チェックより前に判定する)。新規ファイル(`_redirects`等)は
...
#### レース選択グリッド(競馬場×R一覧)(2026-08-16改修・2026-08-17追加修正)
...
- **各レース行(`.race-column-row`)の表示レイアウトはPC・モバイル共通で2行構成にする
  (2026-08-17修正・実装完了)**: PC(700px超)では、`.race-column-row`が「R番号(48px)・
  レース名(1fr)・コース情報/頭数/メモの`<small>`要素(auto)」という3カラム横並びの
  グリッド定義になっていたため、コース情報側の内容量に押されてレース名列が極端に狭くなり、
  文字が実質的に潰れて視認できない不具合があった(PC幅で複数競馬場が横並びカラム表示される
  ぶん1カラムの幅がそもそも狭いため、特に顕著だった)。モバイル(700px以下)は元々
  「R番号(44px)・レース名(残り幅いっぱい)」の2カラム×2行構成(`<small>`要素を
  `grid-column:2`で2行目に固定)になっており正常に表示できていたため、**PC側もこの
  モバイルと同じ2行構成へ統一する**。具体的には、PC幅特有の3カラム横並び定義
  (`grid-template-columns:48px minmax(0,1fr) auto`)を廃止し、モバイル用の2カラム定義
  (`grid-template-columns:44px minmax(0,1fr)`。列幅はPC/モバイルで共通化)と
  `<small>`要素の`grid-column:2`指定を、`@media`で分岐させず常時適用する形にした
  (実装完了)。結果として、PC・モバイルいずれも「1行目: R番号+レース名」「2行目: コース
  情報・頭数・メモマーク」という表示になる。レース名は引き続き画面幅に収まらない場合のみ
  `…`で省略する(全文表示への変更は行っていない)。実装対象は`public/style.css`の
  `.race-column-row`関連ルールのみで、`public/buy.js`のHTML構造(`<b>`/`<span>`/`<small>`の
  3要素構成)は変更していない
...
- **購入画面(`buy.html`)のレース選択グリッド行(`.race-column-row`)**: **2026-08-17〜、
  PC・モバイルとも共通の2行構成にする**(上記「購入画面(`buy.html`)」の「レース選択
  グリッド」節参照。以前はPC幅(700px超)のみ3カラム横並び定義が別途あり、レース名が
  極端に狭くなる不具合があったため、モバイル用の2行構成へ統一した。実装完了)
...
## 返還(refund)処理(2026-08-19〜20設計・実装)
...
`tickets`テーブルに`refunded`カラム(INTEGER、`0`/`1`のブール値。デフォルト`0`)を追加する
(`ALTER TABLE tickets ADD COLUMN refunded INTEGER NOT NULL DEFAULT 0`。既存データに
影響しない非破壊的変更)。
...
`jraResultParseRefund()`が既に解析・保存している`races.payouts.refunds`
...
- 返還判定ロジック(`races.payouts.refunds`と買い目の突き合わせ)は**サーバー側
  ...
  当初は「サーバー・クライアント両方に同じ判定ロジックを実装する」既存方針(コンボ照合
  ロジック向け)を踏襲する想定だったが、返還判定は「購入金額の変更に応じて再計算する
  必要がある値」ではなく「一度確定したら不変の状態」であるため、判定ロジック自体を
  クライアントに複製するとサーバー・クライアントの実装がズレるリスクを増やすだけと
  判断し、確定済みフラグを信頼する設計に変更した(2026-08-20実装時に変更)。
  `computeTicketPayout`は購入履歴・予想登録画面で購入金額を編集した際に払戻額を
  再計算するために呼ばれるが、返還確定済みの馬券は金額編集後も常に新しい購入金額と
  同額を返せば足りるため、この設計で要件を満たせる
- サーバー側の判定は的中判定(`computeWinningCombos`によるコンボ照合)より**先に**行う。
  ...
  判定自体が成立しない)
- `stats.js`の的中率集計(`computeHitRate`)は、`payout > 0`だけで的中とみなす既存ロジックを
  `payout > 0 && !refunded`に変更し、返還を的中としてカウントしないようにする。収支・回収率
  の計算(`computeGroupStats`)は`payout`(返還時は購入金額と同額)をそのまま使うため、
  返還によって収支・回収率が歪むことはない(返還は収支±0になるよう設計されているため)
...
`YYYYMMDDHHmm`形式へ揃えることが望ましい。2026-08-17: レース選択グリッド行のCSS修正に伴い、
`v=2`→`v=3`へ更新済み。**
...
- **モバイル(`700px`以下・2026-08-16〜)**: 縦長になりすぎるのを防ぐため、ヘッダーを2行に
  再構成する。詳細は上記「購入履歴画面(`index.html`)」の「レースカードの表示」節を参照。
  要点のみ再掲する:
