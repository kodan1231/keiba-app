...
土・日     : 払戻一覧PDFをインポート(「JRAレース結果PDFをインポート」。下記「JRAレース結果PDFインポート」参照)
```
...
(`functions/api/races/results-import.js`)へ送信、レース情報・出走馬・着順・払戻をまとめて
登録する。未登録のレースは新規作成し、既存レースは確認のうえ更新できる。
...
- 着順・払戻レートを更新した場合は、そのレースを購入した全ユーザーの`tickets.payout`を
  `recomputeTicketPayoutsForRace()`で再計算・反映する(上記「払戻確定時のticket反映」参照)。
  着順・払戻を書き換えるコードパスを新規に追加する際は、この関数の呼び出しが漏れていないか
  必ず確認すること
...
- `results-import.js`の払戻・着順マージは「既存が完全に空の場合のみ」しか反映しない設計に
  ...
  結果が反映されないことがある(未修正。`docs/BACKLOG.md`「調査中の不具合」参照)
