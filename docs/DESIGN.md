...
- **表示時の組み立て(2026-08-14〜)**: `public/races.js`の`formatCourseText(courseType,
  distance)`が、コース種別・距離をそれぞれ独立した要素として組み立て、**登録されている方だけを
  表示する**(以前は`courseLabel`が空(コース種別未入力)の場合、距離が入力済みでも
...
- コース種別(芝/ダート/障害)・距離(m)は任意入力。レース一覧カードには`courseTypeShort()`で
  ...
  「レース情報のコース種別・距離」節の`formatCourseText()`を参照**
