...
### 払戻モーダルでの手動入力(出走取消馬。2026-09-24追加)

`refunds`はこれまでJRAレース結果PDFインポート(`jraResultParseRefund()`)経由でのみ
自動設定され、レース管理画面から手動で編集する手段が無かった。出走取消の発生自体は
稀だが、発生すると払戻(返還)に直結するため、払戻モーダル(`races-payout-modal.js`)に
手動設定用のUIを追加した。

- **対象は「取消」のみで、「除外」は対象としない**。除外(体重測定後に発走除外となる
  ケース)は、判明した時点で出走馬表(`races.entries`)側から該当馬を取り除く運用のため、
  この時点でまだ`races.entries`に馬番が残っている=購入対象になり得る「取消」とは扱う
  タイミングが異なる(除外は馬番確定〈=購入可能になる〉前に出走馬表側で除去される想定。
  取消は馬番確定後、購入が行われ得る状態で発生する)。そのため払戻モーダルでは
  「出走取消馬」というラベルのみを表示し、除外用の選択肢は設けない
- UI: 払戻モーダルの着順(1〜3着)セレクトの横に「出走取消馬選択」ボタンを置き、
  クリックすると専用ダイアログ(`race-scratched-modal`)が開いて出走頭数
  (`rp-horse-count`)ぶんの馬番チップ(`.horse-chip`。クリックで選択トグル、複数選択可)
  を表示する(2026-09-24。以前は払戻モーダル内に常時表示のチェックボックス欄
  だったが、払戻モーダルが縦に長くなるためダイアログへ切り出した)。出走馬表が
  登録済みなら馬名も表示するが、未登録でも馬番だけで選択できる(着順セレクトと
  同じ考え方)。ボタン自体にも選択頭数(例:「出走取消馬選択(2頭)」)を表示し、
  ダイアログを開かなくても選択状況が分かるようにする。出走頭数を変更した場合、
  新しい頭数の範囲外になった選択は破棄する。選択状態は払戻モーダル側の
  `currentScratchedHorseNumbers`(Set)がそのまま保持するため、ダイアログを
  閉じても消えない(ダイアログ自体はOK/キャンセルの概念を持たない即時反映方式)
- 枠連の返還判定に必要な「返還同枠」(`waku_numbers`)は、出走馬表(`entries`)の枠番情報から
  クライアント側で自動算出する(`computeRefundWakuNumbers()`。ある枠に属する出走馬が
  全頭チェックされていればその枠番を追加)。出走馬表が未登録で枠番情報が無い場合は
  算出できないため常に空配列のままになる(枠連の返還のみ非対応。PDFインポート経由の
  既存動作と同じ制約)
- 保存形式はPDFインポートが生成する`payouts.refunds`と同一(`[{horse_numbers:[...],
  waku_numbers:[...]}]`)のため、`PUT /api/races/:id`・`recomputeTicketPayoutsForRace`
  ともに変更不要でそのまま処理できる。チェック内容は`payouts.refunds`1件にまとめて
  保存する(PDFインポート側は「返還」行ごとに配列要素を分けて追加するが、消費側
  〈`isTicketRefunded`〉は全要素の`horse_numbers`/`waku_numbers`を合算〈union〉して
  判定するだけなので、1件にまとめても複数件でも結果は同じ)
- `race_results`へは反映しない(既存方針通り、このテーブルはPDFインポート経由での
  自動登録専用のまま。上記「画面での参照(レース管理画面)」参照)
- 払戻モーダルを開くと、既存の`payouts.refunds`(PDFインポート由来のものを含む)から
  馬番を読み込みチェック状態に反映する。そのため、PDFインポートで登録された返還内容を
  この画面から確認・修正することもできる
