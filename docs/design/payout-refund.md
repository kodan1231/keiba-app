...
- UI: 払戻モーダルに出走頭数(`rp-horse-count`)ぶんの馬番チェックボックス欄を追加
  (`races-payout-modal.js`の`renderScratchedSection()`)。出走馬表が登録済みなら馬名も
  表示するが、未登録でも馬番だけで選択できる(着順セレクトと同じ考え方)。出走頭数を
  変更した場合、新しい頭数の範囲外になった選択は破棄する
