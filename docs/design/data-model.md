...
| `post_time` | TEXT | 発走時刻 `"HH:MM"`。2026-09-13追加(下記参照) | `"10:35"` |

**発走時刻について(2026-09-13追加)**: 以前は「発走時刻・本賞金・付加賞・調教師名・単勝オッズは
意図的に対象外」としていたが、JRAレース結果ページ(HTML)をユーザースクリプトで取り込む経路
(`docs/design/results-import.md`「ユーザースクリプトによるHTML取込み」参照)を追加したことで
発走時刻だけは副産物として取得できるようになったため撤回し、`post_time`カラムとして保持する
ことにした。出走馬一覧PDF・JRAレース結果PDFにはこの情報が含まれないため、この経路以外からは
値が入らない(既存のPDFインポート経路では常に`NULL`のまま)。馬券購入画面のレース選択グリッドに
表示する(`public/buy.js` `renderGrid()`)。**本賞金・付加賞・調教師名・単勝オッズは引き続き
対象外**(全馬分は結果ページ・結果PDFいずれにも存在しないため。理由は下記「レース結果の詳細記録」参照)。
