...
矛盾・重複記述を整理した。
2026-08-17(2): クラスタI「トラック(競馬場)リストの重複整理」に対応したため本ファイルから削除し、
`archive/documents/BACKLOG_HISTORY.md`へ移動した。同クラスタ「CSSキャッシュバスティングの
一元化」は対象外である旨を明記した)
...
## 🔰 次のチャットで最初に読むこと(2026-08-16(2)引き継ぎ)
...
- **2026-08-16(2)追記: トップページ(`/`)を馬券購入画面にする対応方式を修正した。** 同日先に
  実施した「`functions/_middleware.js`で`/`を`/buy.html`へHTTPリダイレクトする」方式は、
  Cloudflare Pagesの「`*.html`付きURLへのアクセスを拡張子なしURLへ自動的に308リダイレクトする」
  標準挙動と衝突し、「馬券履歴」ナビリンク(`index.html`)をクリックしても馬券購入画面に
  飛ばされてしまう不具合が発生した(リダイレクトの連鎖: `/index.html`→`/`→`/buy.html`→`/buy`)。
  この問題を回避するため、`_middleware.js`のリダイレクト処理は削除し、代わりに
  `public/buy.html`を`public/index.html`へ、旧`public/index.html`(購入履歴)を
  `public/history.html`へリネームする方式に変更した。詳細は`docs/DESIGN.md`
  「トップページ(`/`)の表示について」参照。**このリネームにより、本ファイル内の以降の記述
  および過去の作業履歴(`archive/documents/BACKLOG_HISTORY.md`)にある`buy.html`/`index.html`
  という記述は、`index.html`(旧`buy.html`)/`history.html`(旧`index.html`)と読み替えること**

- **🔴最優先: 下記「部首文字正規化の回帰不具合」の実装がまだ完了していない(方針承認済み・
  コード未修正)。次のセッションではまずこれに着手すること。**
...
- 2026-08-16(2): ルートURL(`/`)へのアクセスに関する対応方式を、上記の通り
  ミドルウェアでのHTTPリダイレクトからファイル名リネーム方式へ修正済み
  (`functions/_middleware.js`からリダイレクト処理は削除済み)
...
「日」「月」「土」「発」「走」「馬」等の通常の漢字が、見た目は同じでも**別のUnicodeブロック
(康熙部首・CJK部首補助)**の文字として出力されるケースがあり、これらはNFKC正規化なら標準の
漢字へ変換されるが、2026-08-14修正後の変換では変換されなくなり、「発走時刻」「年」「月」
「日」等の正規表現が軒並み不一致になっていた。
...
| 🔵 実機検証未完了 | 2026-08-16に実装した「確定済みレースへの購入時の払戻即時反映」(`functions/api/tickets/bulk.js`)は、実ブラウザ・実DBでの動作検証が未実施(コードレビューのみ)。次回実機で試す際は`docs/TESTING.md`「4.5.1」該当項目を確認すること | `docs/TESTING.md` |
| 🔵 実機検証未完了 | 2026-08-16(2)に実装した「トップページ(`/`)のファイルリネーム対応」(`public/index.html`・`public/history.html`・`functions/_middleware.js`)は、実ブラウザ・実DBでの動作検証が未実施(コードレビューのみ)。次回実機で試す際は`docs/TESTING.md`「0.1」該当項目を確認し、特に「馬券履歴」ナビリンクが正しく`history.html`へ遷移すること(以前の不具合が再発していないこと)を重点的に確認すること | `docs/TESTING.md` |
