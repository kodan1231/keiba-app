...
- **次に着手するタスク**: 下記「優先順位(コスト × 影響)」の A 段(最有力は N-1 馬番重複防止)。
...
## 優先順位(コスト × 影響で整理・2026-09-08)

着手順の目安。「コスト」= 実装工数 + リスク、「影響」= ユーザー価値 + 影響範囲。

### A. 最優先(コスト小・効果あり)

| タスク | コスト | 影響 | メモ |
|---|---|---|---|
| **N-1 馬番重複防止** | 小(`races-entries-modal.js` 1ファイル・仕様確定) | 中(手入力時の的中判定バグを防ぐ) | 次の着手候補として最有力 |
| **🔵 実機検証3件**(返還 / パーサ分割 / 枠番) | ゼロ(PDF取込・レース登録のついで) | 中(リグレッション検知) | 実運用の機会に確認するだけ |
| **⚠️ 払戻の矢印区切り 検証** | 極小(実PDFの払戻表を1つ見る + regex 1箇所) | 未確定(馬単/三連単の払戻取りこぼしなら大) | まず「JRA払戻表が `→` を使うか」を確認 |

### B. 中(価値あり・中コスト)

| タスク | コスト | 影響 | メモ |
|---|---|---|---|
| クラスタA 履歴の一括削除 | 中(一括削除API新設。tickets / ticket-imports の2系統に注意) | 中(日常UX) | 単独で切り出し可 |
| クラスタC「名前クリックでドリルダウン」×2(騎手別・レース別) | 中(既存集計データをモーダル表示するだけ) | 中(集計画面の使い勝手) | 4階層化・コース別より軽い。先にこちらだけでも可 |

### C. 大(大規模・要相談・依存あり)

| タスク | コスト | 影響 | メモ |
|---|---|---|---|
| クラスタC「レース別4階層化」「コース別収支」 | 大(集計ロジック再構築) | 大(集計は本アプリの中核) | **ROADMAP クラスタM(race_resultsベース集計)との役割分担を先に決める** |
| クラスタD 未登録レース登録のモーダル化 | 大 | 中(admin のみ・頻度低) | 設計方針を要相談 |

### D. 低・条件待ち

| タスク | 保留理由 |
|---|---|
| N-4 条件詳細カラム表示 | UI相談コストの割に「表示するだけ」。優先度低 |
| クラスタB グリッド微調整 | まず実装済みか実画面で確認 |
| ⚠️ payout マージ all-or-nothing / CSV再計算されない / 騎手名連結 / 降着・失格 status | 発生頻度低・回避策あり・未確証のいずれか |
| ⚠️/クラスタE CSV返還行 payout | サンプルCSV入手が前提 |
| ROADMAP クラスタM | `race_results` にデータが貯まってから |

### やらない

N-3(ログアウト401・実害ほぼ無し)/ ROADMAP クラスタI(CSSキャッシュ・ビルド前提)/
ROADMAP クラスタH(外部データ取得・技術/規約制約)
