...
- 2026-08-19: JRAレース結果PDFインポートで「中止」馬(発走後にレース中に競走を中止した馬)の
  ...
  馬番一覧がズレる不具合があった)。**新たに「返還(refund)処理」を未解決タスクとして
  追加した(下記「クラスタO」参照)。設計方針は未確定で、実装もこれから**
- 次のセッションでは**クラスタN-1(出走馬表の馬番重複入力防止)**、または**クラスタO
  (返還処理の設計)**への着手を推奨する
...
| 🟡 未対応(新規・2026-08-19追加) | **返還(refund)処理が未実装**。「返還馬番」「返還同枠」の情報は`jraResultParseRefund()`で抽出され`races.payouts.refunds`に保存されるが、この情報を使って該当する組み合わせ(取消・除外・中止馬が絡む馬連・ワイド・馬単等)の払戻を「不的中(0円)」ではなく「返還(全額払戻)」として計算する仕組みが無い。枠番が未確定(null)の場合の枠連は既存ロジックで「判定不能(未確定のまま)」として救済されるが、馬番が確定している式別ではこの問題を回避できない。設計方針(どの式別・どの組み合わせを返還対象とみなすか、`recomputeTicketPayoutsForRace`にどう組み込むか)から検討が必要。詳細は`docs/DESIGN.md`「JRAレース結果PDFインポート」既知の制約参照 | `docs/DESIGN.md`「JRAレース結果PDFインポート」既知の制約・下記「クラスタO」 |
...
## クラスタO: 返還(refund)処理の設計(2026-08-19新規追加)

**背景**: JRAでは、出走取消・競走除外・競走中止となった馬が絡む組み合わせ馬券(その馬を
含む馬連・馬単・ワイド・枠連・三連複・三連単等)は、「不的中」ではなく「返還」扱いとなり、
購入金額が全額払い戻される。現状このアプリには返還という概念自体が存在せず、該当する
馬券は的中判定ロジック上「不的中(0円)」として扱われてしまう可能性が高い(枠番未確定の
枠連のみ、既存の「判定不能(null)」ロジックにより偶然救済される)。

**現状把握できている情報**:
- JRAレース結果PDFの「返還」行(例: `返還 返還馬番 8番, 10番 返還同枠 5枠, 6枠`)は
  `jraResultParseRefund()`で解析され、`races.payouts.refunds`に
  `[{horse_numbers:[...], waku_numbers:[...]}, ...]`の形で保存されている(実装済み)
- ただしこの情報は**保存されるだけで、払戻計算(`computeWinningCombos`・
  `computeTicketPayout`・`recomputeTicketPayoutsForRace`)のいずれからも参照されていない**

**検討が必要な論点(未着手)**:
1. どの式別・どの組み合わせを「返還対象」と判定するか(返還馬番を含む組み合わせすべてか、
   枠連の場合は返還同枠を含む組み合わせも対象か等、JRAの実際の返還ルールの確認が必要)
2. 返還時の払戻額は「購入金額と同額(収支±0)」でよいか、的中扱いにするか(現状の
   `tickets.payout`は「的中時の払戻金額」という意味を持つため、返還を的中と区別する
   新しい状態(例: `payout`とは別に`refunded`フラグを設ける等)が必要かもしれない)
3. `stats.js`の的中率集計・収支集計で、返還された馬券をどう扱うか(不的中カウントに
   含めない、収支計算では購入額・払戻額を相殺する、等)
4. CSVインポート側(Club JRA-Net購入履歴)には既に「返還」の概念が存在する可能性がある
   (上記「調査中の不具合」の「的中／返還」列の表)。CSV側の返還処理と統一的な設計に
   できるか確認する

**対応方針**: 着手前にユーザーと上記論点をすり合わせ、`docs/DESIGN.md`へ仕様として
明記してから実装する。規模はデータモデルの拡張(`tickets`/`imported_ticket_items`への
カラム追加の要否を含む)次第で中〜大程度になる見込み。
