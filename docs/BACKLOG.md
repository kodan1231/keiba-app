...
| 🔵 実機検証未完了 | 出走馬一覧PDFの取消・除外の馬の取り込み修正(2026-09-27。行頭が「取消」の行が壊れた出走馬〈馬名「取消 ニシノドリーマー」・騎手名に調教師名が混入〉として取り込まれていた。出走馬には含めず`scratched_horses`に記録、馬番は直前+1で推定、枠番計算では頭数に数える〈取消後も枠番は割り当て直されないため〉、払戻モーダルの頭数初期値をmax(頭数,最大馬番,8)に。`jra-entries-pdf.js`・`entries-merge.js`・`entries-import.js`・`races-payout-modal.js`)は、ユーザー提示のPDFテキスト〈2026-09-27中山1R〉でのパース結果と枠番計算〈JRAの実際の枠と一致〉のみ確認。実PDFのPDF.js抽出結果ではなく提示テキストに基づく想定。**実ブラウザで**: 該当PDFを取り込み、1Rが15頭・取消馬が登録されず・枠番が正しい〈1:1,2 / 2:3,4 / 3:5 / 4:7,8 …〉こと、登録完了メッセージに取消馬が出ること、既に出走馬表にいる場合の表示、払戻モーダルで16番まで選べることを確認する。**未対応の判断待ち**: 既に出走馬表にいる取消馬を自動で外すか、取消を自動で返還〈payouts.refunds〉に設定するか | `docs/design/entries-import.md`「取消・除外の馬」 |
| 🔵 実機検証未完了 | 購入画面の三連複/三連単の通常・フォーメーションの選択ボックスが、iPhone SE3で縦に折り返す不具合を修正(2026-09-26。**原因の推定**: 横にはみ出す要素があるとiOS Safariが表示を縮小して実効幅が430pxを超え、`@media(max-width:430px)`のルールが効かず、`max-width:700px`側の固定38pxの操作列+折り返し可(flex-wrap:wrap)が使われていた。2頭軸の重なりも同根の可能性。実機で幅が430pxを超えていることは未確認〈推定〉。対応: `max-width:700px`側で操作列を`auto`幅・`flex-wrap:nowrap`・各項目`flex:0 0 auto`にし、430px以下のルールと同じ状態を700px以下でも保つ。2頭軸の行も700px以下で同様)は、Edgeで実効幅375px・504pxの両方を再現し〈全式別×方式で1行34px・重なり0〉のみ確認。**実機で**: 三連複/三連単の通常・フォーメーション・2頭軸流しが1行に収まり重ならないことを確認する。根本の「何が横にはみ出してiOSが縮小表示しているか」は未特定 | `public/style.css`「モバイル」`max-width:700px`ブロック |
| 🔵 実機検証未完了 | 購入画面の2頭軸流し(三連単の3列〈1着軸/2着軸/相手〉・三連複の2列)で、iPhone SE3幅(375px)でラジオ/チェックと小ラベルが互いに重なる不具合を修正(2026-09-26。`@media(max-width:430px)`の操作列を固定幅140px/92px+`min-width:0`から、`auto`幅+各項目`flex:0 0 auto; min-width:30px`に変更。はみ出す分は馬名列が省略で吸収)は、375px幅のiframeでのヘッドレスEdge計測〈重なり0・馬名も省略なしで表示〉のみ。実機iPhoneでは未確認〈実機のネイティブなラジオは大きくEdgeと描画が異なるため〉。**実機で**: 三連単/三連複の2頭軸流しで各列が重ならず押しやすいこと、馬名の省略が過剰でないことを確認する | `public/style.css`「2頭軸(軸/相手チェック)の行専用レイアウト」 |
| 🔵 実機検証未完了 | 購入画面のレース一覧に勝負レースの★/☆トグルを追加(2026-09-25。各行の右端のボタンで`PUT /api/predictions/key-race`。従来のレース名後ろの表示のみ「★」は廃止。`public/buy.js`の`toggleKeyRaceFromGrid()`・`.race-row-wrap`/`.key-race-mini`)は`node --check`のみ。**実ブラウザで**: ★/☆を押すとON/OFFが即切り替わりリロード後も保持されること、押してもレース行の遷移(予想画面へ)が起きないこと、予想登録画面のトグルと状態が一致すること、スマホ幅で行の文字と重ならないことを確認する | `docs/design/screens.md`「購入画面のレース一覧」 |
| 🔵 実機検証未完了 | 予想登録画面のUI変更(2026-09-25。馬メモを展開パネルから「メモ」ボタン〈印セレクトの左〉+ダイアログ〈`horse-memo-modal`。自動保存・閉じる時に即時保存〉へ移し、展開パネルは過去成績のみに。メモ登録済みは「メモ」ボタンを赤+「●」で強調〈旧「▼」マークの置き換え〉、過去成績0走は「過去成績はありません」。`public/prediction.js`・`prediction.html`)は、実ページのDOM+コードのヘッドレスEdgeシナリオ〈ボタン状態・ダイアログ・保存・展開〉と狭い幅のスクリーンショットのみ確認。**実ブラウザで**: メモの入力→閉じる→再度開いて保持されていること、別レースの同じ馬にも反映されること(馬名キー)、メモ削除でボタンの強調が消えること、スマホ幅でメモボタン+印セレクトが見切れないことを確認する | `docs/design/screens.md`「予想登録画面」 |
| 🔵 実機検証未完了 | 総合成績タブのフィルタに「JRA」「地方」を追加(2026-09-25。競馬場名で判定〈中央10場=JRA、それ以外の非空=地方〉。`public/stats.js`の`filterByCategory()`・`JRA_TRACKS`。レース別タブには未追加)は`node --check`のみ。**実ブラウザで**: 「JRA」「地方」を押すとサマリー・購入比率バーが該当分だけに絞られること、両者の合計が「すべて」と一致すること(競馬場名が空の購入を除く)を確認する | `docs/design/screens.md`「集計画面」 |
| 🔵 実機検証未完了 | 枠連の購入UIを枠番ベースに変更(2026-09-25。従来は馬選択UIで馬に紐づけて買う形だった。行は枠ごと〈出走馬併記〉、通常/ボックス/流し/フォーメーションとも枠番の組み合わせ、同枠は2頭以上の枠のみ。selectionsはhorse_number・waku_numberとも枠番、馬名・騎手は空。`backfillHorseNamesForRace()`は枠連を対象外に)は、実際の購入モーダルDOM+コードのヘッドレスEdgeシナリオ〈ボックス・流し・通常の組み合わせと生成されるselections〉のみ確認。**実ブラウザで**: 枠連の各購入方式で点数・組み合わせが想定どおりか(特に同枠の扱い)、かご→購入→履歴画面/かご画面の表示が「1-3」等の枠番表示になること、払戻確定後に払戻が正しく計算されること(枠連の払戻レートは枠番の組み合わせ)、出走馬表の枠番未登録レースで案内が出ることを確認する | `docs/design/screens.md`「購入画面」 |
| 🔵 実機検証未完了 | 払戻モーダルの「払戻一括取込」(2026-09-24。楽天競馬・netkeibaの払戻表を貼り付けて式別ごとの払戻と着順を一括設定。`public/payout-paste-parser.js`の`parsePayoutPaste()`。ユーザー提示のサンプル2件でのパース結果と、実モーダルのコード+ヘッドレスEdgeでの取込シナリオ〈netkeibaサンプル→各入力欄・着順・頭数の設定〉は確認済み。実サイトからのコピー結果の形は貼り付け例に基づく想定)は実ブラウザ・実データ未確認。**実ブラウザで**: 楽天競馬・netkeibaの実ページから払戻表をコピー→貼り付けで各式別が設定されること、枠番登録済みのレースで枠連も入ること、取込後に「保存する」で`races.payouts`が更新され購入馬券の払戻が再計算されること、地方競馬の枠単が「取込対象外」と表示されることを確認する | `docs/design/screens.md`「払戻一括取込」 |
| 🔵 実機検証未完了 | 払戻モーダルでの出走取消馬の手動設定(2026-09-24。当初`races-payout-modal.js`に馬番チェックボックス欄を常時表示していたが、払戻モーダルが縦に長くなるため、着順セレクトの横の「出走取消馬選択」ボタン→専用ダイアログ〈`race-scratched-modal`。`.horse-chip`クリックトグルで複数選択〉方式に変更。ボタンには選択頭数を表示。`races.payouts.refunds`を手動編集できるようにする仕組み自体は変更なし。除外は出走馬表確定前に取り除く運用のため対象外、取消のみ扱う。枠連の返還同枠〈`waku_numbers`〉は出走馬表の枠番情報からクライアント側で自動算出〈`computeRefundWakuNumbers()`〉。バックエンド〈`PUT /api/races/:id`・`recomputeTicketPayoutsForRace`〉は無変更で既存の返還判定ロジックがそのまま処理する)は`node --check`のみ。**実ブラウザで**: 「出走取消馬選択」ボタンからダイアログが開き馬番チップをクリックで複数選択できること、ボタンの表示が選択頭数に応じて変わること、ダイアログを閉じても選択状態が保持されること、ダイアログを開いたまま払戻モーダルをキャンセル/ESC/背景クリックで閉じようとした場合にまずダイアログだけが閉じること、保存すると該当馬が絡む馬券(単勝・複勝・馬連等)の`tickets.payout`が購入金額と同額(`refunded=1`)になること、枠連は該当枠の全頭を選択した場合のみ返還になること、出走馬表未登録のレースでも馬番ベースの返還が反映されること、PDFインポート済みレースを開いた際に既存の返還内容が選択済みで表示されることを確認する | `docs/design/payout-refund.md`「払戻モーダルでの手動入力(出走取消馬)」 |
