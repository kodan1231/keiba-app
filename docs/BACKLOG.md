...
| 🟠 未修正(バグ確定・要対応) | 枠番自動計算が **7頭以下のレースで誤った枠番**を返す。`computeWakuNumberFromHorseNumber()`(`functions/api/_lib/entries-merge.js`)と `defaultWakuNumber()`(`public/races-entries-modal.js`)は同一アルゴリズムで、`horseCount<=7` のとき `base=0` になり、若い枠から順に「0頭」が割り当たって全体が後ろへずれる(例: 5頭→枠4,5,6,7,8 になる。正しくは 1〜5)。8頭以上は正常。修正は「`horseCount<=8` なら `waku=horseNumber`」の早期リターン追加(2ファイル)。既に誤った値でDB保存済みのレースは再インポートしても `waku_number` が非nullなら上書きされないため、別途補正が必要か要検討 | `docs/design/data-model.md`「枠番は馬番から自動計算して保存する」 |
