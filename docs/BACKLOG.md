...
- **未適用のマイグレーションは無い**(2026-09-12確認)。`race_results_horse_key`・
  `race_stats_cache` はいずれも本番DB `keiba-yosou-db` へ適用済み・`schema_migrations` 記録済み。
  `horse_key` の既存行バックフィルも admin画面のボタン実行で完了(`race_results` 14768件更新)。
