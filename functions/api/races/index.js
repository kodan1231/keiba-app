...
export async function onRequestPost(context) {
  ...
  const { race_date, track, race_number, race_name, entries } = data;
  ...
  try {
    const result = await env.DB.prepare(
      `INSERT INTO races (race_date, track, race_number, race_name, entries)
       VALUES (?, ?, ?, ?, ?)`
    )
      .bind(race_date, track, race_number, race_name || null, JSON.stringify(entries))
      .run();
