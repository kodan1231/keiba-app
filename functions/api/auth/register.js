import { hashPassword, createSessionToken, readJsonBody, jsonError } from "../_shared.js";
...
export async function onRequestPost(context) {
  ...
  if (!USERNAME_RE.test(username)) {
    return jsonError("ユーザー名は半角英数字とアンダースコアのみ、3〜20文字で入力してください", 400);
  }
  if (password.length < 8) {
    return jsonError("パスワードは8文字以上で入力してください", 400);
  }

  const existing = await env.DB.prepare("SELECT id FROM users WHERE username = ?").bind(username).first();
  if (existing) {
    return jsonError("そのユーザー名は既に使われています", 409);
  }
