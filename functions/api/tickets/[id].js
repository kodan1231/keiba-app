...
export async function onRequestPut(context) {
  ...
  if (!ticket) return jsonError("購入履歴が見つかりません", 404);
  // 通常購入は着順・払戻の確定有無にかかわらず常に編集できる(ロック無し。docs/design/race-edit-lock.md「ロック仕様」参照)。
  const fields=[]; const values=[];
  for (const key of EDITABLE_FIELDS) {
    if (key in data) {
      if (key === "amount" && (!Number.isInteger(Number(data[key])) || Number(data[key]) <= 0)) return jsonError("購入金額が不正です", 400);
      if (key === "payout" && data[key] !== null && (!Number.isInteger(Number(data[key])) || Number(data[key]) < 0)) return jsonError("払戻金額が不正です", 400);
