...
//
// レース数は年間150件程度で自然にバウンドされるため、db.batch() 1回で収まる
// (CLAUDE.mdの「1クエリ100バインドパラメータ」上限・サブリクエスト数上限のいずれにも該当しない)。
export async function onRequestPost(context) {
  ...
  // 既存行をname_keyでまとめて取得し、UPDATE/INSERTを振り分ける。
  ...
  const { results: existingRows } = await env.DB
    ...
    .bind(...nameKeys)
    .all();
  const existingIdByKey = new Map((existingRows || []).map((r) => [r.name_key, r.id]));
