...
3. **購入履歴** (`index.html`) : 登録した購入(通常購入+CSVインポート分)をレース別・購入グループ別に一覧表示(読み取り専用。払戻入力はレース管理で行う)。PCではCSVインポート機能あり(スマホでは非表示)
4. **集計** (`stats.html`) : 総合成績、レース別・競馬場別・騎手別の収支・回収率・的中率
5. **レース管理** (`races.html`) : 出走馬表の登録・編集、開催日程の一括登録、払戻金額の入力(管理者のみ)
...
public/
  ...
  races.html / races.js     ... レース管理(出走馬表・開催日程一括登録・払戻入力。管理者のみ)
...
functions/
  ...
  api/_shared.js             ... 共通処理(管理者判定・馬名バックフィル等)
  api/auth/                  ... ログイン/ログアウト/認証確認/新規ユーザー登録
  api/races/                 ... レースの一覧・登録・編集・削除・一括登録(bulk)。登録・編集・削除は管理者限定
