...
**D1 の100バインドパラメータ上限による障害の修正(2026-09-08)**
- 直前の「コース別収支」対応(6ff0c53)で `GET /api/ticket-imports` に
  `SELECT ... FROM races WHERE id IN (?,?,…)` を追加した際、渡す race_id の数が
  ユーザーの全取込グループぶんに増え、D1 の「1クエリ100バインドパラメータ」上限を超過。
  クエリ失敗 → GET 500 → CSV取込分が履歴画面・集計画面から丸ごと消える障害が発生した
- 修正: `IN (...)` をやめ `SELECT id, course_type, distance FROM races` 全件を1回引いて
  メモリ照合する方式に(races は件数が少ない。全 items を1回SELECTするのと同じ考え方)
- 同時に `tickets/bulk-delete.js` の CHUNK_SIZE を 100→90 に(id 100 + user_id 1 = 101 で
  上限超過していた)
- 横断チェックの結果、他の `IN (...)` は「1レースの出走馬」「かご1回」「取込PDF1本のレース」等で
  自然にバウンドされており、通常運用では上限に触れないと判断。`tickets/bulk.js` と
  `races/[id].js` の該当箇所に「この前提が崩れる変更をする場合はチャンク分割が必要」の
  コメントを追加。共通チャンクヘルパーの導入は「起きない入力のための常設コード」になるため見送り
- `CLAUDE.md`「絶対に破ってはいけない不変条件」に100パラメータ上限の項を追加
