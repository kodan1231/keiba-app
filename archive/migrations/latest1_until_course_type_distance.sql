...
--
-- これは 2026-08-03 に latest1.sql を軽量化する前の、旧latest1.sqlの内容の
-- アーカイブです。以下の2ステップは、本番DB(keiba-yosou-db)へ適用済み・
-- schema_migrationsへの記録も完了しており、その最終結果はschema.sqlに
-- 統合されています。
--
--   - legacy_v13_multiuser   (適用日: 2026-08-03)
--   - course_type_distance   (適用日: 2026-08-03)
--
-- このファイルは実行を想定していません(通常のセットアップ・更新では使用しない)。
-- 万一、このアプリの別の古いDB環境(このアーカイブ作成時点で上記2ステップが
-- 未適用のもの)が存在することが後から判明した場合にのみ、内容を参照して
-- 手動で追いつかせるために保管しています。

-- @STEP: legacy_v13_multiuser
-- v3相当の既存DBを、複数ユーザー対応(v13相当)まで一気に更新するブロック。
-- 旧番号付きmigration(v2〜v13)を順番に適用した最終状態を再現する。
-- テーブルのDROP/RENAMEを伴う一度限りの変換のため、schema_migrationsに
-- 記録された後は二度と実行されない(再実行するとuser_idがNULLに巻き戻る等、
-- データを破壊するおそれがあるため)。
...
-- @STEP: course_type_distance
-- 2026-08-02: races にコース種別・距離を追加(いずれも任意入力。既存行はNULLのまま)
ALTER TABLE races ADD COLUMN course_type TEXT;
